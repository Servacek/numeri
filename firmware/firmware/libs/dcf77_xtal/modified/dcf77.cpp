...
namespace Internal {
    namespace Generic_1_kHz_Generator {
        ...
        // #if defined(__AVR_ATtiny24__) || defined(__AVR_ATtiny44__) || defined(__AVR_ATtiny84__)
            // Calculate OCR1A for 1 kHz: at 16MHz, OCR1A=249; at 8MHz, OCR1A=124.¨
            const uint16_t OCR1A_standard = 124;
            const uint16_t OCR1A_slower   = 125;
            const uint16_t OCR1A_faster   = 123;
            const uint16_t inverse_timer_resolution = 32000uL;
            #if (F_CPU == 16000000L)
                const uint16_t OCR1A_standard = 249;
                const uint16_t OCR1A_slower   = 250;
                const uint16_t OCR1A_faster   = 248;
                const uint16_t inverse_timer_resolution = 64000uL;  // = F_CPU/250
            #elif (F_CPU == 8000000L)
                const uint16_t OCR1A_standard = 124;
                const uint16_t OCR1A_slower   = 125;
                const uint16_t OCR1A_faster   = 123;
                const uint16_t inverse_timer_resolution = 32000uL;
            #elif (F_CPU == 6553500L)
                const uint16_t OCR1A_standard = 102;
                const uint16_t OCR1A_slower   = 103;
                const uint16_t OCR1A_faster   = 101;
                const uint16_t inverse_timer_resolution = 26214uL;
            // #else
            //     #error "Unsupported F_CPU for ATtiny: only 8MHz,16MHz (and 6.5536MHz optionally) supported."
            #endif
        // #endif
        ...
        void init_timer_1() {
            ...
            OCR1A = OCR1A_standard;
            TIMSK1 |= (1<<OCIE1A);  // enable compare match A interrupt
        }

        // Override setup to use Timer1 instead of Timer2, and do NOT disable Timer0.
        void setup(const Clock::input_provider_t input_provider) {
            init_timer_1();
            // Do NOT call stop_timer_0(): keep Timer0/millis() intact on ATtiny.
            the_input_provider = input_provider;
        }

        // ISR for Timer1 Compare Match A calls the generic handler.
        // (Placed outside namespace after )
        // Will be added below in the global scope.

        // Phase-adjustment handler: write to OCR1A instead of OCR0A.
        void isr_handler() {
            cumulated_phase_deviation += adjust_pp16m;
            if (cumulated_phase_deviation >= inverse_timer_resolution) {
                cumulated_phase_deviation -= inverse_timer_resolution;
