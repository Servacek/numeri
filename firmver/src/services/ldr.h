...
static constexpr uint16_t  LDR_ADC_MIN = 0;
static constexpr uint16_t  LDR_ADC_MAX = 800;
...
#if LDR_ENABLED
    if (Config::get(Config::DISPLAY_BRIGHTNESS_MODE) != Config::BRIGHTNESS_AUTO) return;
    if (!Clock::canUseAutoBrightness()) return;
