...
void setup() {
    ...
    // // WGM02=1 (TCCR0B), WGM01=1, WGM00=1 (TCCR0A) -> mode 7 Fast PWM, TOP = OCR0A
    // // COM0B1=1, COM0B0=1 -> inverting mode on OC0B (PD5)
    // TCCR0A = (1 << COM0B1) | (1 << COM0B0) | (1 << WGM01) | (1 << WGM00);

    // // WGM02 and prescaler = 1 (CS00)
    ...
    // Example: Fast PWM, non-inverting on OC0A, prescaler = 1 (safe, less impact on timing)
    TCCR0A = (1 << COM0A1) | (1 << COM0A0) | (1 << WGM00); // inverting, WGM0[1:0]=3 => fast PWM (TOP=0xFF)
    TCCR0B = (1 << CS00); // CS00 -> prescaler 1
    // PWM frequency = 16MHz / (1 * 256) = 62 500 Hz
...
void loop() {
    ...
    // TODO: Nastavit jas ledky
    // setBrightness(map(analogRead(A0), 0, 1023, 0, 255));
